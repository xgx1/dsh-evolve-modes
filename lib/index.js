import { s as evolutionStateSchema, t as DEFAULT_EVOLUTION_CONFIG } from "./schema-Bm6TwqZO.js";
	...
	async recordLearningResult(run, drafts) {
		return this.mutate((state, now) => {
			...
			for (const draft of drafts) {
				...
				if (!state.config.autoApply && proposals.filter((item) => item.status === "pending").length >= state.config.maxPendingProposals) continue;
			...
			if (state.config.autoApply) for (const id of created) {
		...
		scope.effect(() => scope.commands.register({
			...
			handler: async ({ agent, rawInput }) => {
				...
				if (autoApplyMatch !== null) {
					...
					if (autoApply !== "on" && autoApply !== "off") return {
						kind: "error",
						text: "evolve-mode evolution auto-apply expects on or off"
					};
					await evolutionStore.setConfig({
						...evolutionStore.config(),
						autoApply: autoApply === "on"
					});
					return {
						kind: "success",
						text: stateText(agent)
